...
0.4.5 提示词重写模型超时、限流或返回无效内容时，自动使用原提示词和已确认语义槽继续生成，并记录降级状态。工作流映射无效、原始内容不足或用户取消时不会强行提交；不会再次调用模型或重复生图。

0.4.4 提示词重写使用实际槽位 JSON 示例，兼容省略包装、唯一节点输入别名和空负面词。缺省负面词保留已有约束；缺少必要内容、重复或未知字段仍会明确报错，不自动重新生成。
